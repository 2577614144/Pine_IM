...
import com.lipine.im.sdk.utils.AppStationDetection;
...
public class LPIMClient {
    ...
    public void init(Context context) {
        if (context == null) {
            ...
            //应用前后台检测
            AppStationDetection.detectionFrontAndBack();
        }
    }
    ...
    public void login(String serviceIp, int port,String userName, String passWord) throws UnsupportedEncodingException {
        if(channel.isActive() && channel.isOpen()){
            MessageProtobuf.Msg loginMsg =  MessageProcessor.getHandshakeAndLoginMsg(userName,passWord,LPIMConfig.SERVER_IP, LPIMConfig.SERVER_PORT);
            channel.writeAndFlush(loginMsg).addListener(new ChannelFutureListener() {
                @Override
