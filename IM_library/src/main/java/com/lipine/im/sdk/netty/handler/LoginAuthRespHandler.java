...
public class LoginAuthRespHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        MessageProtobuf.Msg resultMsg  = (MessageProtobuf.Msg) msg;
        if (resultMsg == null) {
            return;
        }
        /**
         * 如果为登录消息
         */
        if(resultMsg.getDataType() == MessageProtobuf.Msg.DataType.LoginResponseMsgType){
            MessageProtobuf.LogResponseMsg logResponseMsg =  resultMsg.getLoginResponseMsg();
            LogUtils.eTag(TAG,"================================="+logResponseMsg);
            CEventCenter.dispatchEvent(Events.CHAT_LOGIN_MESSAGE, 0, 0, logResponseMsg);
        }else{
            // 消息透传
...
//          if (MessageType.HANDSHAKE.getMsgType() == loginMsg.getHead().getMsgType()) {
//            int status = -1;
//            try {
//                JSONObject jsonObj = JSON.parseObject(loginMsg.getHead().getExtend());
